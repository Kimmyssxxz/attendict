<template>
  <div class="min-h-screen bg-gray-50 flex flex-col items-center py-8 px-4 md:py-16 selection:bg-[#133e75]/10">
    ...
    <div class="w-full max-w-4xl px-2">
      <!-- Agency Header - Responsive -->
      <header class="w-full mb-6 bg-white rounded-2xl border border-gray-100 p-6 md:p-8 flex flex-col md:flex-row items-center justify-center text-center md:text-left gap-6 md:gap-10">
        <div class="flex items-center justify-center gap-4">
          <img src="/dictlogo2.png" alt="DICT Logo" class="h-16 md:h-20 object-contain opacity-90" />
          <div class="hidden md:block h-12 w-px bg-gray-200"></div>
          <img src="/Bagongpilipinas.png" alt="Bagong Pilipinas Logo" class="h-16 md:h-20 object-contain opacity-90" />
        </div>
        <div class="space-y-1">
          <p class="text-[10px] md:text-xs font-bold tracking-[0.2em] md:tracking-[0.3em] text-gray-400 uppercase">Republic of the Philippines</p>
          <h1 class="text-xs md:text-sm font-bold text-gray-700 tracking-tight uppercase">DEPARTMENT OF INFORMATION AND COMMUNICATIONS TECHNOLOGY</h1>
          <p class="text-gray-500 text-xs md:text-sm font-semibold">DICT ORIENTAL MINDORO</p>
        </div>
      </header>

      <!-- Main Form Card -->
      <div class="w-full bg-white rounded-2xl border border-gray-100 p-6 md:p-12 shadow-sm overflow-hidden">
        <div class="text-center md:text-left pb-6 md:pb-8 border-b border-gray-50 mb-8">
          <h2 class="text-3xl md:text-5xl font-bold text-[#133e75] tracking-tight">Client Logbook</h2>
          <p class="text-gray-400 text-sm md:text-base font-medium mt-1">Digital Registration System</p>
        </div>

        <form @submit.prevent="submitForm" class="space-y-8 md:space-y-10">
          <!-- Basic Info -->
          <div class="space-y-6">
            <div class="flex items-center gap-3 mb-2 pt-4 border-t border-gray-50">
              <div class="hidden md:block w-1.5 h-6 bg-[#133e75] rounded-full"></div>
              <h3 class="text-xl md:text-3xl font-semibold text-gray-800">Basic Information</h3>
            </div>
            
            <div class="space-y-6">
              <div>
                <label for="fullName" class="block text-sm md:text-lg font-medium text-gray-500 mb-2">Full Name</label>
                <input id="fullName" v-model="form.fullName" type="text" placeholder="Name" class="w-full px-5 py-3 md:py-4 bg-white border border-gray-200 rounded-xl focus:outline-none focus:border-[#133e75] transition-all text-base md:text-lg font-medium" />
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <label for="age" class="block text-sm md:text-lg font-medium text-gray-500 mb-2">Age</label>
                  <input id="age" v-model="form.age" type="number" class="w-full px-5 py-3 md:py-4 bg-white border border-gray-200 rounded-xl text-base md:text-lg font-medium" />
                </div>
                <div>
                  <label for="gender" class="block text-sm md:text-lg font-medium text-gray-500 mb-2">Gender</label>
                  <div class="relative custom-dropdown">
                    <button 
                      type="button" 
                      @click.stop="toggleDropdown('gender')"
                      class="w-full px-5 py-3 md:py-4 bg-white border border-gray-200 rounded-xl text-base md:text-lg font-medium text-left flex justify-between items-center focus:outline-none focus:border-[#133e75]"
                    >
          ...
          <div class="space-y-6 pt-4 border-t border-gray-50">
            <div class="flex items-center gap-3 mb-2">
              <div class="hidden md:block w-1.5 h-6 bg-[#133e75] rounded-full"></div>
              <h3 class="text-xl md:text-3xl font-semibold text-gray-800">Contact & Address</h3>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <label for="phone" class="block text-sm md:text-lg font-medium text-gray-500 mb-2">Phone</label>
                <input id="phone" v-model="form.mobileNumber" type="tel" placeholder="09XX" class="w-full px-5 py-3 md:py-4 bg-white border border-gray-200 rounded-xl text-base md:text-lg font-medium" />
              </div>
              <div>
                <label for="email" class="block text-sm md:text-lg font-medium text-gray-500 mb-2">Email</label>
                <input id="email" v-model="form.email" type="email" class="w-full px-5 py-3 md:py-4 bg-white border border-gray-200 rounded-xl text-base md:text-lg font-medium" />
              </div>
              <div>
                <label class="block text-sm md:text-lg font-medium text-gray-500 mb-2">City</label>
                <div class="relative custom-dropdown">
                  <button 
                    type="button" 
              ...
              <div>
                <label class="block text-sm md:text-lg font-medium text-gray-500 mb-2">Barangay</label>
                <div class="relative custom-dropdown">
                  <button 
                    type="button" 
                    :disabled="!form.city"
                    @click.stop="toggleDropdown('barangay')"
                    class="w-full px-5 py-3 md:py-4 bg-white border border-gray-200 rounded-xl text-base md:text-lg font-medium text-left flex justify-between items-center focus:outline-none focus:border-[#133e75] disabled:opacity-50"
                  >
          ...
          <div class="space-y-6 pt-4 border-t border-gray-50">
            <div class="flex items-center gap-3 mb-2">
              <div class="hidden md:block w-1.5 h-6 bg-[#133e75] rounded-full"></div>
              <h3 class="text-xl md:text-3xl font-semibold text-gray-800">Services</h3>
            </div>
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <label v-for="opt in serviceOptions" :key="opt" :class="['flex items-center gap-3 px-4 py-3 md:py-4 rounded-xl border text-sm md:text-lg font-semibold cursor-pointer transition-all', form.services.includes(opt) ? 'border-[#133e75] bg-[#133e75]/5 text-[#133e75]' : 'border-gray-100 bg-white hover:bg-gray-50']">
                <input v-model="form.services" type="checkbox" :value="opt" class="w-4 md:w-5 h-4 md:h-5 rounded text-[#133e75]" />
                {{ opt }}
              </label>
            </div>
            <div class="mt-6">
              <label class="block text-sm md:text-lg font-medium text-gray-500 mb-2">Sector</label>
              <div class="relative custom-dropdown">
                <button 
                  type="button" 
          ...
          <div class="pt-8 border-t border-gray-100">
            <div class="bg-gray-50 rounded-2xl p-6 md:p-8 space-y-6">
              <div class="flex items-start gap-4 text-base md:text-lg text-gray-600 font-semibold">
                <input v-model="form.dataPrivacy" type="checkbox" class="mt-1 w-5 h-5 md:w-6 md:h-6 rounded border-gray-300 text-[#133e75]" />
              ...
              <label :class="['flex items-center gap-3 px-5 py-3 md:py-4 rounded-xl border bg-white cursor-pointer active:scale-[0.99] transition-all', form.agreementConfirmation === 'yes' ? 'border-[#133e75] bg-[#133e75]/5 text-[#133e75]' : 'border-gray-100']">
                <input type="radio" value="yes" v-model="form.agreementConfirmation" class="w-4 md:w-5 h-4 md:h-5 text-[#133e75]" />
                <span class="text-base md:text-lg font-semibold">I read and accept terms</span>
              </label>
              <button type="submit" :disabled="!isFormComplete || submitting" class="w-full bg-[#133e75] text-white text-lg md:text-xl font-bold py-4 md:py-6 rounded-xl shadow-xl active:scale-95 disabled:opacity-50 transition-all">
...
// Removed manual viewport detection for true CSS responsiveness
const handleResize = () => {}

...
export default {
  ...
  methods: {
    ...
    async verifyOtp() {
      ...
      this.isLoading = true;
      try {
        const response = await fetch(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:3001'}/auth/verify-otp`, {
          method: 'POST',
    ...
    async resendOtp() {
      ...
      try {
        const response = await fetch(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:3001'}/auth/resend-otp`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
